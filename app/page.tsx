...
function GoalsScreen({ onNewGoal }: any) {
  return (
    <div className="space-y-8 animate-in slide-in-from-bottom-4 duration-500">
      ...
      <button onClick={onNewGoal} className="neo-button w-full h-[56px] bg-black text-white flex items-center justify-center gap-2">
        ...
        {step === 1 && (
            <div className="space-y-8 pb-20 animate-in fade-in duration-300">
                <div className="bg-[#FFE500] neo-border neo-shadow p-10 text-center space-y-4">
                    ...
                    <div className="space-y-1">
                        <p className="text-[10px] font-black uppercase tracking-widest text-black/40 italic">USDC on Base</p>
                ...
                <div className="space-y-3">
                    <p className="text-[10px] font-black uppercase tracking-widest text-zinc-500 italic">Depositing Into</p>
                ...
                <div className="neo-card neo-shadow p-6 bg-[#C8E6C9] rotate-[1deg] space-y-4">
                    <div className="flex justify-between items-center border-b-2 border-black/10 pb-2">
                        <span className="text-[9px] font-black uppercase tracking-[2px] text-black/40 italic">At Current APY</span>
                        <TrendingUp size={14} className="text-black/40" />
                    </div>
                    <div className="space-y-1 text-left">
                        <p className="text-sm font-black uppercase tracking-widest italic">In 12 Months: <span className="text-xl">$1,086.00</span></p>
                        <p className="text-[10px] font-bold uppercase text-black/60 italic">Yield Earned: $86.00</p>
                    </div>
                </div>

                <button onClick={() => setStep(2)} className="neo-button w-full h-[64px] bg-[#FFE500] text-sm tracking-[2px] font-black uppercase leading-none italic">Confirm Deposit ${amount}</button>
            </div>
        )}
        ...
        {step === 2 && (
            <div className="flex-1 flex flex-col items-center justify-center space-y-10 animate-in zoom-in-95 duration-500">
                ...
                <div className="text-center space-y-2">
                    <h3 className="text-4xl font-black uppercase tracking-tighter italic">Depositing ${amount}</h3>
                    <p className="text-sm font-black uppercase tracking-widest text-zinc-500">Into yoUSD Vault</p>
                </div>
                <div className="w-full max-w-xs space-y-3">
                    <div className="h-6 w-full bg-black neo-border overflow-hidden">
                        <motion.div 
                            initial={{ width: 0 }} 
                            animate={{ width: '60%' }} 
                            className="h-full bg-[#FFE500] border-r-3 border-black" 
                        />
                    </div>
                    <p className="text-center text-[10px] font-black uppercase tracking-widest animate-pulse italic">Approving USDC...</p>
                </div>
            </div>
        )}

        {step === 3 && (
            <div className="flex-1 flex flex-col items-center justify-center space-y-12 animate-in slide-in-from-bottom-12 duration-500">
                <div className="text-center space-y-4">
                    <h2 className="text-7xl font-black uppercase tracking-tighter italic leading-none">Deposited! 🎉</h2>
                    <div className="bg-[#C8E6C9] neo-border neo-shadow px-6 py-2 rotate-[-1deg] inline-block font-black text-xl italic uppercase tracking-tighter">
                        ${amount}.00 → yoUSD
                    </div>
                </div>

                <div className="neo-card neo-shadow p-8 bg-white w-full space-y-4">
                    <p className="text-[10px] font-black uppercase tracking-widest text-zinc-400 text-center italic">Stats Update</p>
                    <div className="flex justify-between items-end border-b-2 border-black pb-4">
                        <span className="text-sm font-black uppercase italic">New Total</span>
                        <span className="text-3xl font-black italic">$1,347.83</span>
                    </div>
                    <div className="flex justify-between items-center text-[10px] font-black uppercase italic text-[var(--positive)]">
                ...
                <div className="grid grid-cols-2 gap-4 w-full">
                    <button onClick={onClose} className="neo-button h-[64px] bg-white text-sm tracking-widest italic font-black uppercase">View Vault</button>
                    <button onClick={onShare} className="neo-button h-[64px] bg-[#FFE500] text-sm tracking-widest italic font-black uppercase flex items-center justify-center gap-2">
                ...
                <div className="space-y-6 pb-20 animate-in fade-in duration-300 text-left">
                    <div className="bg-[#FFE500] neo-border neo-shadow p-6 space-y-4">
                        <div className="flex justify-between items-start">
                            <p className="text-[10px] font-black uppercase tracking-widest leading-none italic">What are you saving for?</p>
                            <span className="text-[9px] font-black text-black/30 font-mono italic">{name.length}/30</span>
                        </div>
                        <input 
                            value={name}
                            onChange={(e) => setName(e.target.value.toUpperCase())}
                            className="w-full bg-white neo-border p-4 text-sm font-black uppercase italic outline-none placeholder-zinc-300" 
                            placeholder="EMERGENCY FUND, NEW LAPTOP..." 
                        />
                    </div>

                    <div className="neo-card neo-shadow space-y-4 bg-white">
                        <p className="text-[10px] font-black uppercase tracking-widest leading-none italic text-zinc-500">Target Amount</p>
                        ...
                        <div className="grid grid-cols-3 gap-2">
                            {['USDC', 'ETH', 'BTC'].map(c => (
                                <button key={c} className={cn("neo-border py-2 text-[10px] font-black uppercase italic", c === 'USDC' && "bg-[#FFE500]")}>{c}</button>
                            ))}
                        </div>
                    </div>

                    <div className="neo-card neo-shadow space-y-4 bg-white">
                        <p className="text-[10px] font-black uppercase tracking-widest leading-none italic text-zinc-500">Target Date</p>
                        <div className="grid grid-cols-2 gap-3">
                            <div className="neo-border p-4 flex justify-between items-center cursor-pointer">
                                <span className="text-sm font-black italic">SEP</span>
                                <ChevronDown size={16} strokeWidth={3} />
                            </div>
                            <div className="neo-border p-4 flex justify-between items-center cursor-pointer">
                                <span className="text-sm font-black italic">2026</span>
                                <ChevronDown size={16} strokeWidth={3} />
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="neo-card neo-shadow bg-[#C8E6C9] p-6 rotate-[2deg] flex items-center gap-6">
                        <div className="flex-1 space-y-1">
                            <p className="text-[8px] font-black uppercase tracking-[2px] text-black/40 italic leading-none">Recommended Vault</p>
                            <h4 className="text-2xl font-black italic tracking-tighter italic">yoUSD <span className="bg-[#FFE500] text-[8px] px-2 py-0.5 rounded-full border-2 border-black inline-block ml-2 align-middle">8.6% APY</span></h4>
                            <p className="text-[9px] font-bold uppercase text-black/60 italic tracking-tight">Best for stable savings goals under 24 months</p>
                        </div>
                    </div>

                    <button onClick={() => setStep(2)} className="neo-button w-full h-[64px] bg-[#FFE500] text-sm tracking-widest italic font-black uppercase mt-4">Generate Plan</button>
                </div>
            )}

            {step === 2 && (
                <div className="space-y-8 pb-20 animate-in slide-in-from-bottom-8 duration-500 text-left">
                    <div className="bg-black text-white neo-border neo-shadow p-8 space-y-4">
                        <p className="text-[10px] font-black uppercase tracking-widest text-white/40 italic">To hit $5,000 by Sep 2026</p>
                        <h2 className="text-7xl font-black italic tracking-tighter text-[#FFE500] italic leading-none">$387/mo</h2>
                        <p className="text-[9px] font-bold uppercase text-white/60 italic tracking-widest">Or deposit $3,200 today as lump sum</p>
                    </div>
                    ...
                    <div className="neo-border border-dashed p-6 bg-[#FF4D4D]/10 rotate-[-1deg] space-y-2">
                        <div className="flex items-center gap-2 text-[#FF4D4D] font-black uppercase text-sm tracking-tighter italic">
                            <ShieldAlert size={18} /> Tight Timeline
                        </div>
                        <p className="text-xs font-bold uppercase text-zinc-500 leading-tight italic">Increase monthly deposit or extend deadline to reduce stress on your portfolio.</p>
                    </div>

                    <button onClick={() => setStep(3)} className="neo-button w-full h-[64px] bg-[#FFE500] text-sm tracking-widest italic font-black uppercase">Create Goal</button>
                </div>
            )}

            {step === 3 && (
                <div className="flex-1 flex flex-col items-center justify-center space-y-12 animate-in zoom-in-95 duration-500">
                    <h2 className="text-5xl font-black uppercase tracking-tighter italic leading-none italic">Goal Created! 🎯</h2>
                    <div className="w-full">
                        <GoalCard title={name || "SAVINGS GOAL"} target="$5,000" progress={0} current="$0" rotate="1" tag="yoUSD VAULT" />
                    </div>
                    <button onClick={onClose} className="neo-button w-full h-[64px] bg-[#FFE500] text-sm tracking-widest italic font-black uppercase">Start Saving Now</button>
                </div>
            )}
        </OverlayWrapper>
    );
    ...
    return (
        <OverlayWrapper title="Badge Gallery" onClose={onClose}>
            <div className="space-y-8 pb-20">
                <div className="bg-[#FFE500] neo-border neo-shadow p-10 text-left space-y-4">
                    <div className="flex items-center gap-4">
                        <Flame size={48} strokeWidth={3} />
                        <h3 className="text-3xl font-black uppercase tracking-tighter italic italic">14 Week Streak</h3>
                    </div>
                    <div className="space-y-2">
                        <p className="text-[10px] font-black uppercase tracking-widest italic leading-none opacity-60">Keep going — 12 weeks to next badge</p>
                        <div className="h-4 w-full bg-black/10 neo-border overflow-hidden">
                            <div className="h-full bg-black border-r-2 border-black" style={{ width: '55%' }} />
                        </div>
                    </div>
                </div>
                    ...
                    {badges.map((b, i) => (
                        <div 
                            ...
                            className={cn(
                                ...
                                b.unlocked ? "bg-white cursor-pointer hover:translate-y-[-2px]" : "bg-zinc-100 opacity-40 border-dashed"
                            )}
                        >
                            <span className={cn("text-4xl", !b.unlocked && "grayscale")}>{b.rank}</span>
                            <div className="space-y-1">
                                <p className="text-[9px] font-black uppercase tracking-widest leading-none">{b.name}</p>
                                <p className="text-[7px] font-black uppercase tracking-widest text-zinc-400 leading-none">{b.status}</p>
                            </div>
                        </div>
                    ))}
                </div>
...
function BadgeDetailOverlay({ badge, onClose, onShare }: any) {
    return (
        <OverlayWrapper title="Badge Detail" onClose={onClose}>
            <div className="flex-1 flex flex-col items-center justify-center space-y-10 py-10 animate-in zoom-in-95 duration-500">
                ...
                <div className="text-center space-y-2">
                    <h3 className="text-5xl font-black uppercase tracking-tighter italic italic leading-none">{badge.name}</h3>
                    <p className="text-sm font-black uppercase tracking-[3px] text-zinc-500">{badge.desc}</p>
                    <div className="bg-black text-white px-4 py-1.5 rounded-full inline-block mt-4 text-[10px] font-black uppercase tracking-widest italic">
                        Unlocked {badge.date}
                    </div>
                </div>
                <button onClick={onShare} className="neo-button w-full h-[64px] bg-[#FFE500] flex items-center justify-center gap-3 text-sm tracking-widest italic font-black uppercase italic">
                    Share on X <Share2 size={18} />
                </button>
            </div>
        </OverlayWrapper>
        ...
        <OverlayWrapper title="Share to X" onClose={onClose} bgColor={style === 'BLACK' ? '#121212' : '#F5F0E8'}>
            <div className="space-y-10 pb-20 animate-in fade-in duration-500">
                <div className={cn(
                    ...
                    <div className="flex justify-between items-start">
                         <span className="text-2xl font-black uppercase tracking-tighter italic italic leading-none">VAULTA</span>
                         <Globe size={24} strokeWidth={3} />
                    </div>
                    
                    <div className="space-y-2 text-center">
                        <h2 className="text-6xl font-black uppercase tracking-tighter italic italic leading-none">🔥 14 WEEK STREAK</h2>
                        <p className="text-sm font-black uppercase tracking-[2px] opacity-60 italic">Earning 8.6% APY on Base</p>
                    </div>

                    <div className="flex justify-between items-end">
                        <span className="text-[10px] font-black uppercase tracking-widest italic">vaulta.xyz</span>
                        <div className="flex items-center gap-2">
                             <div className="w-2 h-2 rounded-full bg-blue-500 animate-pulse" />
                             <span className="text-[8px] font-black uppercase tracking-widest italic">Base Network</span>
                        </div>
                    </div>
                </div>
                ...
                <div className="space-y-6 text-left">
                    <div className="space-y-3">
                        <p className="text-[10px] font-black uppercase tracking-widest text-zinc-500 italic leading-none">Card Style</p>
                        <div className="grid grid-cols-3 gap-2">
                            {['YELLOW', 'BLACK', 'WHITE'].map(s => (
                                <button key={s} onClick={() => setStyle(s)} className={cn("neo-border py-4 text-[10px] font-black uppercase italic", style === s && "bg-[#FFE500] text-black")}>{s}</button>
                            ))}
                        </div>
                    </div>

                    <div className="space-y-4">
                        <div className="flex items-center justify-between text-[10px] font-black uppercase italic text-zinc-500">
                            <span>Customise</span>
                        </div>
                        <div className="space-y-3">
                             <ToggleRow label="Show Balance" active />
                             <ToggleRow label="Show Wallet" />
                        </div>
                    </div>

                    <div className="neo-card bg-white/20 p-5 space-y-2 border-dashed border-zinc-400">
                        <p className="text-[8px] font-black uppercase text-zinc-400 italic">Preview Copy</p>
                        <p className="text-xs font-bold uppercase leading-tight italic">Saving is a team sport. Just hit my 14 week streak on @VaultaHQ! Earning 8.6% APY on my stablecoins. 🚀 #DeFi #Base</p>
                    </div>
                </div>

                <button className="neo-button w-full h-[64px] bg-[#00ACEE] text-white text-sm tracking-widest italic font-black uppercase flex items-center justify-center gap-3">
                    Post to X <ExternalLink size={18} />
                </button>
            </div>
        </OverlayWrapper>
    );
        ...
        <div className="space-y-10 pb-20 text-left">
            <div className="bg-black text-white neo-border neo-shadow p-6 rotate-[-1deg] space-y-2">
                <div className="flex items-center gap-3 mb-2">
                    ...
                    <p className="text-xl font-black uppercase tracking-tighter italic">If you disappear</p>
                </div>
            ...
            <section className="space-y-6">
                <h3 className="text-sm font-black uppercase tracking-[3px] border-b-3 border-black pb-1 inline-block italic">Set up your Will</h3>
                <div className="space-y-4">
                    <div className="neo-card neo-shadow space-y-2 bg-white">
                        <p className="text-[10px] font-black uppercase tracking-widest text-zinc-500 italic">Beneficiary Wallet</p>
                        <input className="w-full bg-[#F5F0E8] neo-border p-4 text-xs font-black uppercase outline-none italic" placeholder="0x... or ENS name" />
                        <p className="text-[8px] font-bold text-zinc-400 uppercase italic">This wallet receives everything if triggered</p>
                    </div>
                    <div className="neo-card neo-shadow space-y-4 bg-white">
                        ...
                        <div className="grid grid-cols-3 gap-2">
                            {['6 Mo', '12 Mo', '24 Mo'].map(v => (
                                <button key={v} className={cn("neo-border py-3 text-[10px] font-black uppercase italic", v === '12 Mo' && "bg-[#FFE500]")}>{v}</button>
                            ))}
                        </div>
                    ...
                    <div className="neo-card neo-shadow p-5 space-y-4 bg-white">
                        ...
                        <div className="space-y-3">
                             <ToggleRow label="No Deposits" active />
                             <ToggleRow label="No Logins" active />
                        </div>
                    </div>
                </div>
            </section>
            ...
            <div className="neo-border border-dashed p-5 rotate-[1deg] space-y-2 bg-white/50">
                <p className="text-sm font-black uppercase tracking-tight text-[#FF4D4D] flex items-center gap-2 italic">
            ...
            <button className="neo-button w-full h-[64px] bg-black text-[#FFE500] text-sm tracking-widest italic font-black uppercase">Activate Will</button>
        </div>
    </OverlayWrapper>
  );
}
  ...
  return (
    <div className="neo-card neo-shadow space-y-4 bg-white" style={{ transform: `rotate(${rotate}deg)` }}>
      ...
      <div className="pt-2 border-t-2 border-black flex items-center justify-between">
        <span className="text-[10px] font-black uppercase flex items-center gap-1 italic">
          <CheckCircle2 size={12} strokeWidth={3} className="text-green-600" /> On Track
        </span>
        <span className="text-[8px] text-zinc-400 font-bold uppercase italic italic">Est. Sep 2026</span>
      </div>
    </div>
  );
}
...
function MilestoneBadge({ rank, label, unlocked, rotate, locked, onClick }: any) {
  return (
    <div onClick={onClick} className={cn(
      ...
      {unlocked ? (
        <div className="bg-black text-white text-[7px] font-black px-1.5 py-0.5 rounded-full uppercase italic">Unlocked</div>
      ) : (
        <div className="text-[7px] font-black uppercase opacity-60 italic">Locked</div>
      )}
    </div>
  );
}
  ...
  return (
    <div className="neo-card neo-shadow p-6 space-y-3 relative group overflow-hidden bg-white">
      <div className="flex justify-between items-start">
        ...
        <span className="text-[8px] font-black px-2 py-0.5 border-2 border-black rounded-full uppercase italic" style={{ backgroundColor: color }}>
          {badge}
        </span>
      </div>
        ...
        <div className="neo-card neo-shadow p-6 space-y-6" style={{ backgroundColor: color }}>
            <div className="flex items-center justify-between">
                ...
                <div className="text-right">
                    <p className="text-sm font-black uppercase italic italic leading-none">{name}</p>
                    <p className="text-[9px] font-mono font-bold text-zinc-400 mt-1">{wallet}</p>
                </div>
            </div>
            <div className="grid grid-cols-2 gap-4">
                <div className="neo-border p-3 text-center">
                    <p className="text-[7px] font-black uppercase text-zinc-400 tracking-widest mb-1 italic">Sending</p>
                    <p className="text-sm font-black uppercase italic italic">{amount}</p>
                </div>
                <div className="neo-border p-3 text-center">
                    <p className="text-[7px] font-black uppercase text-zinc-400 tracking-widest mb-1 italic">Since</p>
                    <p className="text-sm font-black uppercase italic italic">{since}</p>
                </div>
            </div>
            <div className="flex justify-between items-center pt-2">
                <span className={cn("text-[9px] font-black px-3 py-1 rounded-full border-2 border-black uppercase tracking-widest italic", color === "#FFFFFF" ? "bg-[#FF4D4D] text-white" : "bg-black text-white")}>Active</span>
                <button className="text-[9px] font-black uppercase tracking-widest text-zinc-400 underline underline-offset-4 italic">Cancel</button>
            </div>
        </div>
    );
}
...
function ProfileStat({ label, value }: any) {
  return (
    <div className="neo-card neo-shadow p-4 space-y-1 bg-white">
      <p className="text-[8px] text-zinc-500 font-black uppercase tracking-widest leading-none italic">{label}</p>
      <p className="text-xl font-black italic tracking-tighter leading-none italic">{value}</p>
    </div>
  );
}
...
function ToggleRow({ label, active }: any) {
    ...
    return (
        <div className="flex items-center justify-between">
            <span className="text-[10px] font-black uppercase tracking-widest italic text-zinc-500">{label}</span>
            <button onClick={() => setIsOn(!isOn)} className="w-10 h-6 neo-border bg-[#F5F0E8] relative">
                <motion.div animate={{ x: isOn ? 18 : 2 }} className="absolute top-1 w-3 h-3 bg-black" />
            </button>
        </div>
    );
}
...
function SimpleStat({ label, value }: any) {
    return (
        <div className="neo-card neo-shadow bg-white p-3 text-center space-y-1">
            <p className="text-[7px] font-black uppercase text-zinc-400 italic leading-none">{label}</p>
            <p className="text-[10px] font-black uppercase italic italic leading-none">{value}</p>
        </div>
    );
}
