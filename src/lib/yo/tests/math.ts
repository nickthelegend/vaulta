...
export const mathTests = [
  {
    ...
    group: 'Math',
    run: async () => {
      const result = mathUtils.calculateYieldEarned(BigInt("100000000"), BigInt("108600000"));
      if (result === BigInt("8600000")) return { status: 'PASS', value: 'calculateYieldEarned: 8.60 USDC ✓' };
  ...
  },
  {
    ...
    group: 'Math',
    run: async () => {
      const result = mathUtils.formatUSDC(BigInt("1247830000"));
  ...
  },
  {
    ...
    run: async () => {
      ...
      if (result === BigInt("1247830000")) return { status: 'PASS', value: 'parseUSDC: 1247830000n ✓' };
  ...
  },
  {
    ...
    group: 'Math',
    run: async () => {
      const result = mathUtils.applySlippage(BigInt("1000000000"), 50);
      if (result === BigInt("995000000")) return { status: 'PASS', value: 'applySlippage 0.5%: 995000000n ✓' };
